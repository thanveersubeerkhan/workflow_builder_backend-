...
authRouter.get('/connect/:service', (req, res) => {
  ...
  if (!SERVICE_SCOPES[service] && service !== 'github' && service !== 'microsoft' && service !== 'outlook' && service !== 'excel' && service !== 'word' && service !== 'teams' && service !== 'onedrive') {
  ...
  if (service === 'github') {
  ...
  } else if (['microsoft', 'outlook', 'excel', 'word', 'onedrive'].includes(service)) {
     url = getMicrosoftAuthUrl(userId as string, callbackUrl as string, service);
  } else {
     url = client.generateAuthUrl({
      ...
      console.log(`[OAuth] Generated URL for ${service}:`, url);
      console.log(`[OAuth] Requested Scopes for ${service}:`, [
          ...(SERVICE_SCOPES[service] || []), 
          ...SERVICE_SCOPES.identity
      ]);
  }
...
authRouter.get('/callback/:service', async (req: any, res: any) => {
  ...
  try {
    ...
    // Allow 'microsoft' callback to serve all Microsoft-related services
    const isMicrosoftService = ['outlook', 'excel', 'word', 'onedrive', 'microsoft'].includes(stateService);
    const effectiveService = (service === 'microsoft' && isMicrosoftService) ? stateService : service;

    if (effectiveService !== stateService) {
    ...
    if (effectiveService === 'github') {
      ...
      tokens = {
        ...
        expiry_date: Date.now() + (1 * 60 * 60 * 1000), // 1 hour absolute
    ...
    } else if (['microsoft', 'outlook', 'excel', 'word', 'teams', 'onedrive'].includes(effectiveService)) {
        const msData = await getMicrosoftAccessToken(code as string, effectiveService);
        console.log(`Microsoft data (${effectiveService}):`, msData.microsoftUser);

        tokens = {
            access_token: msData.access_token,
            refresh_token: msData.refresh_token,
            expiry_date: Date.now() + (msData.expires_in ? msData.expires_in * 1000 : 3600 * 1000),
            scope: msData.scope
        };
      ...
      const client = createOAuthClient(`/auth/callback/${effectiveService}`);
    ...
    if (!tokens.refresh_token && effectiveService !== 'github') {
